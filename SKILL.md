...
1. **Read the sources** — the request as stated, any existing decision or design
   ...
   which decision governs which cycle.
...
4. **Fill `template.md`** into `docs/superpowers/cycles/<topic>.md` — undated, unlike
...
5. **Create the status board** (see The Status Board below): author
   `docs/superpowers/cycles/<topic>-board.html`, publish it as an artifact, and write
   the returned URL into the cycles file's `**Board:**` header line.
6. **Commit both files.**
...
2. **The status board** — edit `docs/superpowers/cycles/<topic>-board.html` to match,
   and republish it passing `url:` = the address in the cycles file's `**Board:**`
   line. A transition without a board refresh, or a republish without that `url:`, is
   an incomplete update.

## Mode 3 — Resume

Invoked with the argument `continue` (also `resume` / `retomar`, or any ask to
pick a program back up in a fresh session). Read the cycles files in
`docs/superpowers/cycles/`; if more than one program has cycles not `done`,
ask which. Then scan that program's cycles and act on the **highest row of
...
| `spec` | The spec awaits your human partner's review — ask for it; once approved, invoke superpowers:writing-plans. |
...
1. **Reconcile against git.** Commits since the cycles file was last touched; branch and
   merge state per cycle; spec/plan files under `docs/superpowers/` that no `Artifacts`
...
4. **Sweep for session-only state.** Anything load-bearing that lives only in the
   ...
   outside the repo is a dangling pointer: copy the file in, or state plainly that it is
   expendable and why.
...
board, committed at `docs/superpowers/cycles/<topic>-board.html` and published as a
Claude artifact.
...
is not a record — run Mode 4.

## Dispatching a Cycle

Take the first cycle whose dependencies read `done` and invoke
superpowers:brainstorming with that entry as the scope statement. Its `Scope` /
`Out` / `Done when` pre-answer brainstorming's scope check: this is one cycle
because someone already cut it and wrote down where.
